	...
	if (rob.getStatus() != BLESSE)
        ...
        if (rob.getStatus() == BLESSE)
        	...
        	rob2.blessure();
				...
				if(!(rob.getStatus()==DOWN) && rob.getY()>POS_SOL-rob.getSprite().getGlobalBounds().height)
					rob.setPosSprite(rob.getX(),POS_SOL-rob.getSprite().getGlobalBounds().height);
				if(!(rob2.getStatus()==DOWN) && rob2.getY()>POS_SOL-rob2.getSprite().getGlobalBounds().height)
					rob2.setPosSprite(rob2.getX(),POS_SOL-rob2.getSprite().getGlobalBounds().height);
}

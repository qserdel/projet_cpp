...
void Map::create(){
	switch(index){
		case 1:
			if(!fond.loadFromFile("images/fond.jpg",IntRect(100, 300, TAILLE_WINDOW_X, TAILLE_WINDOW_Y)))
      		exit(EXIT_FAILURE);
    	spriteFond.setTexture(fond);
			...
			spriteSol.setTextureRect(IntRect(0, 0, TAILLE_WINDOW_X, 50));
			spriteSol.setPosition(Vector2f(0.f, POS_SOL));
			creation_objets();
		break;
		case 2:
			if(!fond.loadFromFile("images/fond2.png",IntRect(0, 0, TAILLE_WINDOW_X, TAILLE_WINDOW_Y)))
				exit(EXIT_FAILURE);
			spriteFond.setTexture(fond);
		...
		break;
		case 3:
			if(!fond.loadFromFile("images/fond3.jpg",IntRect(0, 0, TAILLE_WINDOW_X, TAILLE_WINDOW_Y)))
				exit(EXIT_FAILURE);
			spriteFond.setTexture(fond);
...
void Map::ajoutCollectable() // Ajoute des collectables de manière aléatoire
{
	srand(time(NULL));
...
void Map::ajouterListeCollec(Collectable *cnew)
{
	c.push_back(cnew);
}
...
Sprite Map::getSpriteObj(int indice) const { return *objets[indice]; };
